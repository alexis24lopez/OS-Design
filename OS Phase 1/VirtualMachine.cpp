...
void VirtualMachine::run(std::fstream& objectCode, std::fstream& in, std::fstream& out)
{
    ...
    while (pc < limit) {
        ...
        if (opcode == 0) {    /* load loadi */
			...
			// Zero out register rd
			r[rd] &= 0x0;

			// Reads the value of the register destination in the .in file
			in >> r[rd];
			clock += 27;
			...
			// Writes the value of the register destination to the .out file
			out << r[rd] << '\n';
